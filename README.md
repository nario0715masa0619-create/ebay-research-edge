...
バージョン V16.0：ヤフーフリマ構造変化対応、ジャンル別手数料DB化、AI漢字キーワード優先版
...
- **Yahoo!フリマ**：最新のグリッド構造に対応した安定スクレイピングロジック
...
- **ジャンル別動的手数料計算**（フィギュア 12.7%、ポケカ 11.35% など、DB により自動切替）
...
- **高精度キーワード抽出**：
  - カード番号（例：110/080）とレアリティ（SAR、AR など）を優先的に抽出
  - **AI 漢字優先化**：作品名やキャラクター名を日本公式の漢字表記（例：潔世一）で抽出し、国内ヒット率を最大化
